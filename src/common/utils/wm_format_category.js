...
export default {
    ...
    formatTreeData(treejson)  {
        ...
        {
            if (formatTreeJson[treejson[i].parent_id]) {
                var x = formatTreeJson[treejson[i].parent_id].i * 1 + 1;
                formatTreeJson[treejson[i].parent_id].i = x;

                //node数以三位递增，不足补0
                treejson[i].node = x*1 < 10 ? ("00" + x):( x*1 > 100 ? x : ("0" + x));
                formatTreeJson[treejson[i].parent_id].children.push(treejson[i]);
            } else if(treejson[i].parent_id === 0){
                ...
                formatTreeJson[treejson[i].parent_id] = {};
                formatTreeJson[treejson[i].parent_id].i = 1;
                formatTreeJson[treejson[i].parent_id].children = [];
                treejson[i].node = "001";
                formatTreeJson[treejson[i].parent_id].children.push(treejson[i]);
            }
        }
